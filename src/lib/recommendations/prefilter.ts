import{evaluateEligibility}from"@/lib/ingestion/eligibility";import type{CandidateData,Job,JobPreferences}from"@/types/job";
...
const seniority=(value:string)=>{const v=value.toLowerCase();if(/principal|staff|director|head|lead/.test(v))return 4;if(/senior|sr\.?/.test(v))return 3;if(/junior|jr\.?|entry|graduate/.test(v))return 1;return 2};
export function prefilterJob(job:Job,preferences:JobPreferences,candidate:Pick<CandidateData,"profile"|"skills">,now=new Date()):PrefilterResult{
  const hard=evaluateEligibility(job,preferences);if(!hard.eligible)return{eligible:false,score:0,reasons:hard.reasons.map(x=>`Excluded: ${x}`)};
  ...
  if(preferences.desiredLocations.some(x=>job.location.toLowerCase().includes(x.toLowerCase()))){score+=7;reasons.push("Preferred location matches")}
  ...
  if(candidate.profile.preferredRoleLevel){const gap=seniority(job.seniority??job.title)-seniority(candidate.profile.preferredRoleLevel);if(gap>=2){score-=20;reasons.push("Role appears substantially more senior")}else if(Math.abs(gap)<=1){score+=6;reasons.push("Seniority appears aligned")}}
