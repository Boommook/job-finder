...
export type InternshipSeason = "any" | "summer" | "fall" | "spring" | "winter";
...
export interface JobPreferences { desiredTitles:string[]; desiredLocations:string[]; allowUsRemote?:boolean; remotePreference:WorkArrangement|"flexible"; employmentTypes:EmploymentType[]; internshipSeason:InternshipSeason; internshipYear:number|null; minimumSalary:number|null; preferredIndustries:string[]; preferredSkills:string[]; excludedKeywords:string[]; sponsorshipRequired:boolean; }
...
export interface CandidatePartialDate { precision:"day"|"month"|"year"|"present"|"unknown"; year:number|null; month:number|null; day:number|null; raw:string|null; }
...
export interface CandidateExperience { id:string; organization:string; title:string; startDate:CandidatePartialDate; endDate:CandidatePartialDate; isCurrent:boolean; location:string; description:string; skills:string[]; source:CandidateSource; confirmed:boolean; }
export interface CandidateProject { id:string; name:string; description:string; technologies:string[]; projectUrl:string; repositoryUrl:string; startDate:CandidatePartialDate; endDate:CandidatePartialDate; source:CandidateSource; confirmed:boolean; }
export interface CandidateEducation { id:string; institution:string; degree:string; programs:string[]; graduationDate:CandidatePartialDate; coursework:string[]; gpa:number|null; source:CandidateSource; confirmed:boolean; }
...
export interface JobQuery { page?:number; pageSize?:number; search?:string; status?:"all"|JobStatus; view?:"all"|"recommended"|"new"|"dismissed"; company?:string; source?:string; arrangement?:"all"|WorkArrangement; sort?:"match"|"posted"|"discovered"; }
